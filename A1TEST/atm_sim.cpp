...
int count =0;
int customerCount=0;
int i=0; // customer count
int atmVariable=0;
ATM * arrayOfATM;
arrayOfATM = new ATM[N];
	...
	while(minute < M ){
		...
		//loop through each ATM and see if they're available
		cout<<"before the for loop--------------------------"<<endl;
		//loop through the customers
		while(atime_list[i] == minute ){
			cout<<"Looping through ATM#:"<<i<<endl;
			if(arrayOfATM[i].returnGetAvailable()==true && i+1<=N)
			{
				//FIRST STEP: put customer into each available ATM first
				Customer * obj;
				obj = new Customer(atime_list[i],ttime_list[i]);
				//
				if(atime_list[i+1]==-1){
					return 0;
				}
				cout<<"i first run:"<<i<<endl;
				arrayOfATM[i].insertCustomer(obj,ttime_list[i]);
				customerCount++;
				//arrayOfATM[i].setAvailable();

				cout<<"inserting a customer into ATM"<<endl;
				cout<<"customer atime:"<<obj->getAT()<<" "<<obj->getTT()<<endl;
				cout<<"ATM IS NOW SET TO"<<arrayOfATM[i].returnGetAvailable()<<endl;
				//EVERY ATM NOW HAS A CUSTOMER
				//if number of customer is more than number of atm, we have to loop again
			}
			else{
				//infinite loop to keep going through each atm
					cout<<"ALL ATM USED ------------------------ NOW ADD TO Q"<<endl;

					Customer * obj;
					obj = new Customer(atime_list[i],ttime_list[i]);
					cout<<"i:"<<i<<endl;
					cout<<"atmVariable:"<<atmVariable<<endl;
					cout<<"BEFORE ATM "<<arrayOfATM[atmVariable].returnGetAvailable()<<endl;
					arrayOfATM[atmVariable].insertCustomer(obj,ttime_list[i]);
					customerCount++;


					//arrayOfATM[atmVariable].setAvailable();
					cout<<"inserting a customer into ATM"<<endl;
					cout<<"customer atime:"<<obj->getAT()<<" "<<obj->getTT()<<endl;
					cout<<"ATM IS NOW SET TO"<<arrayOfATM[atmVariable].returnGetAvailable()<<endl;
					atmVariable++;
					cout<<"queue starting-----"<<atmVariable<<endl;
					if(atmVariable==N)
					{ //keep looping it over and over again
						atmVariable=0;
					}
			}
			i++;
		...
		for(int i = 0; i<N; i++){
			arrayOfATM[i].processCustomer(minute);
			cout<<"PROCESS ATM IS NOW SET TO"<<arrayOfATM[i].returnGetAvailable()<<endl;
		}
...
int getSimParameters(int &M, int &N, int &C, int *&atime_list, int *&ttime_list) {
  ...
  atime_list = new int [C+1]; //start from index 0
  atime_list[C+1]={-1};
